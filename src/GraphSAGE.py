...
class GraphSAGE(nn.Module):
    def __init__(self,
        ...
        # input layer
        self.layers.append(dglnn.SAGEConv(in_dim, num_hidden, aggregator_type, activation=activation))
        # hidden layers
        for i in range(num_layers - 1):
            self.layers.append(dglnn.SAGEConv(num_hidden, num_hidden, aggregator_type, activation=activation))
        # output layer
        self.layers.append(dglnn.SAGEConv(num_hidden, out_dim, aggregator_type))  # activation None
...
def train(args):
    ...
    model = GraphSAGE(graph, args.num_layers, args.dim_node, args.dim_node, args.dim_node,
                      F.relu, args.dropout, "mean", args.time_encoder_type,

...
for data_name in "A" "B" ; do
    ...
    export job_name=${data_name}-CPU

    if [[ ! -v upload_data ]]; then
        amlt run amulet_submit/test_who_can.yaml $exp_name --upload-data  --sku C16
        export upload_data=True
    else
        amlt run amulet_submit/test_who_can.yaml $exp_name  --sku C16
    fi
done
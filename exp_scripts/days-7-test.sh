...
export exp_name=_Test_withou_hup
# export job_name=
export dim_node=128
export dim_time=10
...
for data_name in "A" "B" ; do
    ...
    export job_name=${embedding}_Emb-${time_encoder_type}_TE-${data_name}

    if [[ ! -v upload_data ]]; then
        amlt run amulet_submit/submit.yaml $exp_name --upload-data 
        export upload_data=True
    else
        amlt run amulet_submit/submit.yaml $exp_name
    fi
done
